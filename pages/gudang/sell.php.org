...
if(isset($_POST['button']))
{
	...
	if($rc==1)
	{
		if($_POST['qty'] < $rw['stok_tersedia'])
		{
			...
			if($q)
			{
				$qr=mysql_query("Select sum(jumlah) as jl from barang_keluar Where kode_barang='".$_POST['kodebarang']."'");
				$rw22=mysql_fetch_array($qr);
				
				$q3=mysql_query("Update data_persediaan SET keluar=".$rw22['jl'].",stok_tersedia=stok_tersedia - ".$_POST['qty']." Where kode_barang='".$_POST['kodebarang']."'");
				if($q3)
				{
